...
- (void)configure:(FIRApp *)app providerName:(NSString *)providerName customDebugToken:(NSString *)customDebugToken {
  if ([providerName isEqualToString:@"debug"]) {
    FIRAppCheckDebugProvider *provider = [[FIRAppCheckDebugProvider alloc] initWithApp:app];
    
    // Use custom debug token if provided
    if (customDebugToken != nil && [customDebugToken length] > 0) {
      // Need to use newer interface to set custom debug token
      [provider setValue:customDebugToken forKey:@"debugToken"];
      NSLog(@"Firebase App Check Custom Debug Token: %@", customDebugToken);
    } else {
      // Use default debug token
